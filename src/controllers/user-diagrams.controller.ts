...
export async function getAllUserDiagrams(req: express.Request, res: express.Response) {
  ...
  if (user.type === UserType.Operario) {
    try {
      //let newToken = renewToken(token);
      filters.create_by = user.id_usu;
...
export async function deleteDiagram(req: express.Request, res: express.Response) {
    ...
    if (user.type === UserType.Operario) {
      try {
        //let newToken = renewToken(token);
        let diagram = await diagramsService.remove(Number.parseInt(req.params.id),user.id_usu);
